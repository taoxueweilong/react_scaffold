...
export default class TestResults extends Component {
  ...
  getTimeMenuData(){
    ...
    let params = {
          type: 'GET',
          async : true,
          url: 'healthcabin/checkbody/gettimelist.zb',
          contentType: '',
          data:{
            userId:userId
          }
  ...
  getResultsData(){
    ...
    let params = {
          type: 'GET',
          async : true,
          url: 'healthcabin/checkbody/getBodyCheckResultByTime.zb',
          contentType: '',
          data: {
            time: key,
            userId: userId
        ...
        function success(res){
          that.setState({
            scoreChart: res.scoreChart,//echert数据
            bodyType: res.conclusion.bodyType,//体质
            performance: res.conclusion.performance,//体质描述
            diet: res.solution.diet,//饮食
            motion: res.solution.motion,//运动
            reminder: res.solution.reminder,//温馨提示
            lifeWay: res.solution.lifeWay,//生活方式
          }, function(){
            this.onEcharts();
          })
        };
  ...
  render() {
    let {url, status, yearSeven, yearEight, bodyType, performance, diet, motion, reminder, lifeWay} = this.state;
    let s = null;
    if(status == 0){
      s = history
    } else if(status == 1){
                  ...
                  <ScrollArea height={280}>
                    <div style={styles.allStyle}>
                      <div style={styles.allStyle1}>
                        <p style={styles.jielun}><img src={result} style={styles.result}/><span style={styles.cpjl}>测评结论</span></p>
                      ...
                      <div style={styles.allStyle2}>
                        <p style={styles.tubiao}><img src={result} style={styles.result}/><span style={styles.tbfx}>图表分析</span></p>
