...
class Index extends Component {
  ...
  getQuickData (quickAddData) {
    ...
    let key = quickAddData.medicinename;
    quickAddData.key=key;
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      let medicineArr = dataSource.map((item, index) => {
        nameArr.push(item.orderSuitname)
  ...
  render () {
    ...
    return (
      <div>
        <BasePop visible={visiblePop} title='新增材料单' onClose={() => this.handlePopClose()}>
          ...
          <div style={styles.bodyTop} onClick={()=>{this.hidePop()}}>
              <Form className='customForm not-draggable' id='form'>
                ...
                >
                  ...
                  <Table.Column title="材料明细" dataIndex="orderSuitname" />
                  <Table.Column title="执行科室" dataIndex="deptname" />
                  <Table.Column title="数量/单位" dataIndex="count" render={this.numberCell.bind(this)} />
...
const Material = Form.create()(Index);
