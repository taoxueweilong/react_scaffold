...
import InputEnterPop from 'components/dr/input/enterPop';
...
export default class Diagnose extends Component {
  ...
  initialData(){
    ...
    function callBack(res){
      if(res.result){
        self.setState({
          diagnoseHisData: getDiagnoseHisDataSource(res.data)
        });
      }else{
  ...
  getTableCol(){
    const columns = [{
      ...
      title: '主诊断',
      dataIndex: 'mainDiaType',
      align: 'center',
      className: 'height-24',
      key: 'mainDiaType',
    }, {
      title: '疑似诊断',
      dataIndex: 'doubtDiaType',
      align: 'center',
      className: 'height-24',
      key: 'doubtDiaType',
    }, {
  ...
  addChinaMedicineData(e){
    ...
    if(!symptom.hasOwnProperty('diseaseid')){ // 校验失败
      return;
    }
    ...
    //病症病侯组合
    let diagnoseFinaleInfoItem = JSON.parse(JSON.stringify(symptom));
  ...
  addWestMedicineData(e){
    ...
    let diagnoseDataItem = {
      ...
      mainDiaType: (mainDiagnose)?'是':'否',
      doubtDiaType: (doubleDiagnose)?'是':'否',
      doctorname: window.sessionStorage.getItem('username'),
    };
    ...
    diagnoseFinaleInfoItem.mainDiaType = mainDiagnose;
    diagnoseFinaleInfoItem.doubtDiaType = doubleDiagnose;
