...
import DateSelector from 'components/dr/dateSelector';
import Pagination from 'components/antd/components/pagination';
import ajaxGetResource from 'commonFunc/ajaxGetResource';
import DateFormat from 'commonFunc/dateFormat';
      ...
      <Container onClick={()=>{this.searchPatient.hideResult()}}>
        ...
        <LeftFloat visible={visible}>
          ...
          <Middle>
            ...
            <Button type="primary" className='smaller' onClick={()=>{this.receptionJudge()}}>接诊</Button>
            <Line src={line}/>
            <Button type="primary" className='smaller' onClick={()=>{this.PatientDetailInfo.handleOpen()}}>登记</Button>
          </Middle>
          <Bottom>
            <RadioGroup className='radio-height-40' defaultValue={rcStatus} onChange={(e)=>{this.handleRadioChange(e)}}>
              <Radio value={0} className='span-5'>待接诊患者</Radio>
              <Radio value={1} className='span-5'>已接诊患者</Radio>
            </RadioGroup>
            <DateSelector onSubmit={(date)=>{this.getPatienList(date)}} disabled={rcStatus == 0}></DateSelector>
          </Bottom>
          <Table
            dataSource={patienList}
            rowClassName='patienListTableRow'
            columns={columns}
            className='patienList'
