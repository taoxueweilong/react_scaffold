...
import '../antd/style/button.css';
import '../antd/style/icon.less';
...
export default class Popout extends Component {
  ...
  render() {
    let {visible, children, onOk, title = '精美弹框'} = this.props;
    return (
      <Modal
         isOpen={visible}
         style={modalStyle}
        ...
        <Draggable
          ...
          <Pannel className="handle" onClick={this.cancelHandle}>
            <Header className='header'>
              <span>{title}</span>
              <Icon type='close' className='popout-close' onClick={this.handleClose} />
            </Header>
            <div>
              {children}
            </div>
            {
              ...
              (
                <Footer>
                  <Button type="primary" className='semicircle' onClick={this.handleOK}>确定</Button>
